...
Phase F.1 adds an ephemeral self-correction projection without changing SQLite history. When the Owner questions the immediately preceding answer, `SelfCorrectionPolicy` locates the original Owner request, retains later Owner-authored evidence, and withholds Assistant-authored claims made after that request from the review call. A provenance instruction marks all Assistant output as untrusted context rather than fact and keeps Owner messages and approved Long-Term Memory distinct. Owner-authored evidence is NFKC-normalized only in this ephemeral review projection so equivalent full-width tokens do not select a different verification path; stored and displayed text is untouched. Review calls use deterministic verification decoding instead of conversation sampling so correction does not depend on another seed-sensitive sample; normal conversation generation settings are unchanged. This is task-agnostic and contains no arithmetic answer or expression rule. Numeric input following an Assistant question remains ordinary Owner input rather than being misclassified as a correction.

Gemma channel extraction now decodes generated tokens with special markers intact. `thought` and `analysis` channel content is discarded before the atomic response is persisted; an explicit `final` channel is preferred when present. A thought-only generation receives a safe retry message rather than exposing draft content. Content-free Details telemetry reports whether correction projection or private-channel filtering occurred.
